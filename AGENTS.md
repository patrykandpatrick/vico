...
│   ├── core/                   # Core library (Android-only, shared logic)
...
│   ├── multiplatform/          # Compose Multiplatform support
...
```
**Note:** Requires Java 24 for CI; works with Java 17 locally.

To auto-format files:
```bash
java -jar ktfmt.jar --google-style <file.kt>
```
...
2. `./gradlew test`
3. `java -jar ktfmt.jar --dry-run --google-style --set-exit-if-changed <changed-files>`
...
| Component | Path |
|-----------|------|
| Chart core classes | `vico/core/src/main/java/com/patrykandpatrick/vico/core/cartesian/` |
| Common utilities | `vico/core/src/main/java/com/patrykandpatrick/vico/core/common/` |
| Compose components | `vico/compose/src/main/java/` |
| Multiplatform code | `vico/multiplatform/src/commonMain/kotlin/` |
| Views components | `vico/views/src/main/java/` |
| Unit tests | `vico/core/src/test/java/` |
| Dependency versions | `gradle/libs.versions.toml` |
| Build versions | `buildSrc/src/main/kotlin/Versions.kt` |

## Important Notes

1. **Explicit API mode:** All library modules use `explicitApi()` - public API must have explicit visibility modifiers.

2. **JVM Target:** JVM 11 for all modules.

3. **Android SDK:** compileSdk=36, minSdk=21 (defined in `Versions.kt`).

4. **Multiplatform targets:** Android, iOS (arm64, x64, simulatorArm64), Desktop (JVM), JS, WASM.

5. **Generated files:** The `buildSrc/build/` directory contains generated Gradle accessor files that may fail formatting checks. These are excluded from the source tree and regenerated during builds.

6. **Test framework:** JUnit 5 (Jupiter) with MockK for mocking.

7. **Publishing:** Uses `com.vanniktech.maven.publish` plugin; publishing requires Maven Central credentials.

## Troubleshooting

- **Slow first build:** Initial Gradle/dependency downloads can take 5-10 minutes.
- **Formatting failures on generated files:** Run `./gradlew clean` before formatting checks, or exclude `buildSrc/build/` from ktfmt.
- **Test deprecation warnings:** Some test methods call deprecated APIs - these warnings are expected.
...
- You need to modify files not mentioned here
- The repository structure appears to have changed
