...
@Composable
private fun ComposeBasicDonutChart(
      ...
      rememberPieChart(
        sliceProvider =
          PieChart.SliceProvider.series(
            vicoTheme.pieChartColors.map { color ->
              PieChart.Slice(
                fill = Fill(color),
                label =
...
@Composable
fun ComposeBasicDonutChart(modifier: Modifier = Modifier) {
  val modelProducer = remember { PieChartModelProducer() }
  LaunchedEffect(Unit) { modelProducer.runTransaction { pieModel { series(60, 20, 20) } } }
  ComposeBasicDonutChart(modelProducer, modifier)
}

@Composable
@Preview
fun ComposeBasicDonutChartPreview() {
  val modelProducer = remember { PieChartModelProducer() }
  // Use `runBlocking` only for previews, which don’t support asynchronous execution.
  runBlocking?.invoke { modelProducer.runTransaction { pieModel { series(60, 20, 20) } } }
  PreviewBox { ComposeBasicDonutChart(modelProducer) }
}
