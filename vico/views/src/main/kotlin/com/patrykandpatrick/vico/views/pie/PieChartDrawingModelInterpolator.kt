...
private class DefaultPieChartDrawingModelInterpolator : PieChartDrawingModelInterpolator {
  ...
  override suspend fun transform(fraction: Float): PieChartDrawingModel? {
    ...
    val oldSlices = oldDrawingModel?.slices.orEmpty()
    val newSlices = newModel.slices
    val sliceCount = maxOf(oldSlices.size, newSlices.size)
    return PieChartDrawingModel(
      List(sliceCount) { index ->
        newSlices.getOrNull(index)?.transform(oldSlices.getOrNull(index), fraction)
