...
public abstract class BaseCartesianChartView<Model : ChartEntryModel> internal constructor(
...
) : BaseChartView<Model>(context, attrs, defStyleAttr), ScrollListenerHost {
    ...
    final override var model: Model? = null
        private set
    ...
    /**
     * Sets the [Model] for this [BaseCartesianChartView]’s [Chart] instance ([chart]).
     */
    override fun setModel(model: Model) {
        val oldModel = this.model
        this.model = model
        tryInvalidate(chart = chart, model = model)
        if (isAttachedToWindowCompat && oldModel?.id != model.id && isInEditMode.not()) {
            handler.post {
