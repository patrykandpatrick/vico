...
internal class MotionEventHandler(
  ...
  private val onPointerState: (PointerState) -> Unit,
  ...
  fun handleMotionEvent(motionEvent: MotionEvent, scrollHandler: ScrollHandler): Boolean {
    ...
    return when (motionEvent.action and MotionEvent.ACTION_MASK) {
      MotionEvent.ACTION_DOWN -> {
        ...
        onPointerState(PointerState.Pressed(motionEvent.point))
        lastX = initialX
        currentX = initialX
      ...
      MotionEvent.ACTION_MOVE -> {
        ...
        if (scrollEnabled) {
          ...
          if (shouldPerformScroll && !ignoreEvent) {
            ...
            onPointerState(PointerState.Moved(motionEvent.point))
            requestInvalidate()
      ...
      MotionEvent.ACTION_UP -> {
        totalDragAmount = 0f
        onPointerState(PointerState.Released(motionEvent.point))
