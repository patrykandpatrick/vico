...
internal fun Modifier.chartLayout(
    preMeasureContext: PreMeasureContext,
    legend: Legend?,
    desiredHeight: MeasureScope.(Constraints) -> Int,
) = layout { measurable, constraints ->
    val legendHeight = legend?.getHeight(preMeasureContext, constraints.maxWidth.toFloat())?.toInt() ?: 0
    val height =
        (constraints.minHeight.coerceAtLeast(desiredHeight(constraints)) + legendHeight)
